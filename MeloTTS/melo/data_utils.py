...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, audiopaths_sid_text, hparams):
        """
        metadata를 로드하고 데이터셋 전처리 옵션을 초기화한다.

        Args:
            audiopaths_sid_text: metadata list 파일 경로
            hparams: config의 data 섹션 객체
        """
    ...
    def _filter(self):
        """
        유효하지 않은 샘플을 걸러내고 대략적인 스펙트로그램 길이를 계산한다.
        """
    ...
    def get_audio(self, filename):
        """
        파형을 로드하고 캐시된 스펙트로그램/파형 텐서를 반환한다.

        스펙트로그램 캐시는 wav 옆에 `.spec.pt` 또는 `.mel.pt`로 저장된다.
        """
    ...
    def get_text(self, text, word2ph, phone, tone, language_str, wav_path):
        """
        텍스트 기반 심볼 필드를 텐서로 변환하고 BERT feature를 붙인다.

        BERT feature는 `<wav>.bert.pt` 캐시를 읽거나, 없으면 새로 생성한다.
        """
    ...
    def get_sid(self, sid):
        """speaker id를 shape [1] LongTensor로 변환한다."""
        sid = torch.LongTensor([int(sid)])
        return sid

    def __getitem__(self, index):
        """인덱스로 데이터셋 샘플 하나를 반환한다."""
...
class DistributedBucketSampler(torch.utils.data.distributed.DistributedSampler):
    ...
    def _create_buckets(self):
        """버킷을 만들고 각 버킷을 분산 배치 크기에 맞게 패딩한다."""
    ...
    def __iter__(self):
        """
        길이가 비슷한 샘플 인덱스 배치를 생성한다.

        각 replica는 버킷 패딩된 인덱스에서 서로 겹치지 않는 shard를 받는다.
        """
    ...
    def _bisect(self, x, lo=0, hi=None):
        """길이 `x`의 버킷 인덱스를 반환하고, 범위를 벗어나면 -1을 반환한다."""
        if hi is None:
