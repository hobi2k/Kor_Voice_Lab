...
torchrun --nproc_per_node=1 --master_port=29501 \
  -m melo.train \
  -m testing \
  -c /mnt/d/tts_data/mai_ko/filelists/config.json \
...
# MeloTTS 루트에서 스크립트 직접 실행(`python melo/train.py`)도 가능하도록
# 패키지 루트를 sys.path 앞에 보장한다.
...
def run():
    """
    학습 실행에 필요한 구성요소를 만들고 epoch 루프를 수행한다.

    분산 학습 초기화, 데이터셋/데이터로더 구성, 모델/옵티마이저/스케줄러 구성,
    pretrained 및 체크포인트 로드, epoch별 `train_and_evaluate` 호출까지 담당한다.
    """
...
def train_and_evaluate(
    ...
    for batch_idx, (
        ...
        # Noise-scaled MAS 사용 시 global step 기준으로 스케일을 감쇠시킨다.
        ...
        with autocast(enabled=hps.train.fp16_run):
            ...
            # 먼저 generator 출력을 detach해서 discriminator들을 업데이트한다.
        ...
        with autocast(enabled=hps.train.fp16_run):
            # 이후 adversarial + 재구성 손실로 generator를 업데이트한다.
        ...
        if rank == 0:
            ...
            if global_step % hps.train.eval_interval == 0:
                # 평가 주기에 맞춰 G/D/DUR를 같은 global step으로 저장한다.
...
def evaluate(hps, generator, eval_loader, writer_eval):
    """
    검증 추론을 수행하고 mel/audio 요약을 tensorboard에 기록한다.

    SDP on/off 두 경로를 모두 추론해 비교 가능하도록 기록하며,
    생성 음성과 정답 음성을 함께 로깅한다.
    """
    generator.eval()
    image_dict = {}
    audio_dict = {}
